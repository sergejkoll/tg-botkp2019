...
func getTaskTitle(bot *tgbotapi.BotAPI, userId int64, taskTitle string) {
	...
	// Отправка сообщения
	msg := tgbotapi.NewMessage(userId, "Введите крайний срок выполнения задачи в формате yyyy-dd-mm hh:mm")
	_, err := bot.Send(msg)
	if err != nil {
		log.Fatal(err)
	}
...
func getTaskDeadline(bot *tgbotapi.BotAPI, chatId int64, taskDeadline string) {
	...
	t, err := time.Parse("2006-02-01 15:04", taskDeadline)
	if err != nil {
		log.Fatal(err)
	}
...
>>>>>>> 2abe5836f74dd2984c370d9ce2e1ec1c8d1c367f
	if resp.StatusCode == 200 {
